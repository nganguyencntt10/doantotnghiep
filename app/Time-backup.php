...
class Time
{
	...
	public function get_room($array_key, $data_array, $service_time, $data){
		...
		$time_code 			= 	[];
		$time_room_start 	= 	[];
		for ($i=0; $i < count($service_time); $i++) { 
			// mã thời gian tối đa
			$time_code_max 		= static::time_code_max($service_time[$i]->time);

			// mã thời gian cần thiết
			$time_last 	= count($time) > 0 ? $time[count($time) - 1] : Carbon::now('Asia/Ho_Chi_Minh')->toTimeString();

			$min_next_time 		= static::time_code_next($time_last, $service_time[$i]->time);
			// return ($min_next_time);
			// danh sách code_time trong các phòng
			$time_code_room 	= static::get_array_time_service($data[$i]);

			// return $time_code_room;
			// tìm kiếm khung thời gian phù hợp
			$code_time = $min_next_time;
			// array_push($room, static::time_real($code_time, $service_time[$i]->time));
			while ($code_time < $time_code_max) {
				// code...
				foreach ($time_code_room as $key => $value) {
					if (!in_array($code_time, $value)) {
						// tên phòng
						array_push($room, $data[$i][$key]->name);
						// thời gian kết thúc
						array_push($time, static::time_real($code_time, $service_time[$i]->time));
						// thời gian bắt đầu
						array_push($time_room_start, static::time_real_start($code_time, $service_time[$i]->time));
						// mã thời gian
		...
		// $this->time_room_start = $time_room_start;
